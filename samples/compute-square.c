...
struct ComputeSquareBC {
	HccRwTexture2D(FMT_8_8_8_8_UNORM) output;
};
...
HCC_COMPUTE(8, 8, 1)
void compute_square_cs(HccComputeSV const* const sv, ComputeSquareBC const* addrsp(BC) const bc) {
	...
	store_textureG(bc->output, coord, color);
}
