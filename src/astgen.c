...
bool hcc_astgen_data_type_check_compatible_assignment(HccWorker* w, HccDataType target_data_type, HccASTExpr** source_expr_mut) {
	...
	if (HCC_DATA_TYPE_IS_MUTONLY(source_data_type)) {
		return false;
	}
...
void hcc_astgen_data_type_ensure_readable(HccWorker* w, HccASTExpr* expr) {
	if (HCC_DATA_TYPE_IS_MUTONLY(expr->data_type)) {
		hcc_astgen_bail_error_1_manual(w, HCC_ERROR_CODE_ATTEMPTING_TO_READ_FROM_MUTONLY, expr->location);
	}
}
...
HccATAToken hcc_astgen_ensure_static_assert(HccWorker* w) {
	...
	hcc_astgen_data_type_ensure_readable(w, cond_expr);
...
HccATAToken hcc_astgen_generate_type_specifiers(HccWorker* w, HccLocation* location, HccASTGenTypeSpecifier* type_specifiers_mut) {
	...
	while (1) {
		...
		switch (token) {
			...
			case HCC_ATA_TOKEN_KEYWORD_MUTONLY: found_specifier = HCC_ASTGEN_TYPE_SPECIFIER_MUTONLY; break;
...
NON_NUM_TYPE: {}
		...
		default: {
			...
			if (type_specifiers & HCC_ASTGEN_TYPE_SPECIFIER_CHAR) {
				...
				} else if (type_specifiers & HCC_ASTGEN_TYPE_SPECIFIER_SIGNED) {
	...
	if (type_specifiers & HCC_ASTGEN_TYPE_SPECIFIER_MUTONLY) {
		data_type = HCC_DATA_TYPE_MUTONLY(data_type);
	}
	...
	if (inner_expr->type == HCC_AST_EXPR_TYPE_CONSTANT) {
		switch (unary_op) {
			...
			case HCC_AST_UNARY_OP_NEGATE:
			{
				...
				//
				HccBasic inner_basic = hcc_constant_table_get_basic(w->cu, inner_expr->constant.id);
				HccBasic eval = hcc_basic_eval_unary(w->cu, unary_op, unary_expr_data_type, inner_basic);
				inner_expr->constant.id = hcc_constant_table_deduplicate_basic(w->cu, unary_expr_data_type, &eval);
				return inner_expr;
			}
		}
	}
...
HccASTExpr* hcc_astgen_generate_unary_expr(HccWorker* w) {
	...
	switch (token) {
		...
		case HCC_ATA_TOKEN_STRING: {
			HccStringId string_id = hcc_ata_iter_next_value(w->astgen.token_iter).string_id;

			HccString string = hcc_string_table_get(string_id);
			HccConstantId string_size_constant_id = hcc_constant_table_deduplicate_basic(w->cu, HCC_DATA_TYPE_AST_BASIC_CHAR, (HccBasic*)&string.size);
			HccDataType data_type = hcc_array_data_type_deduplicate(w->cu, HCC_DATA_TYPE_AST_BASIC_CHAR, string_size_constant_id);

			HccASTExpr* expr = hcc_astgen_alloc_expr(w, HCC_AST_EXPR_TYPE_CONSTANT);
			expr->constant.id = hcc_constant_table_deduplicate_bytes(w->cu, data_type, (uint8_t*)string.data, string.size);
			expr->data_type = data_type;
			expr->location = hcc_ata_iter_location(w->astgen.token_iter);
			hcc_ata_iter_next(w->astgen.token_iter);
...
UNARY:
		{
			...
			if (unary_op != HCC_AST_UNARY_OP_ADDRESS_OF) {
				hcc_astgen_data_type_ensure_readable(w, inner_expr);
			}
		...
		case HCC_ATA_TOKEN_CURLY_OPEN: {
			...
			while (1) {
				...
				hcc_astgen_data_type_ensure_readable(w, value_expr);
...
CURLY_INITIALIZER_FINISH: {}
		...
		case HCC_ATA_TOKEN_KEYWORD_GENERIC: {
			...
			while (1) {
				...
				hcc_astgen_data_type_ensure_readable(w, case_stmt->generic_case.expr);
				if (case_stmt_tail) {
...
HccASTExpr* hcc_astgen_generate_call_expr(HccWorker* w, HccASTExpr* function_expr) {
	...
	while (1) {
		...
		hcc_astgen_data_type_ensure_readable(w, arg_expr);
		if (args_count < params_count) {
	...
	if (w->astgen.function) {
		if (function_expr->type != HCC_AST_EXPR_TYPE_FUNCTION || HCC_AML_OPERAND_AUX(function_decl) >= HCC_FUNCTION_IDX_USER_START) {
...
HccASTExpr* hcc_astgen_generate_array_subscript_expr(HccWorker* w, HccASTExpr* array_expr) {
	...
	if (HCC_DATA_TYPE_IS_ARRAY(resolved_data_type)) {
		...
		if (HCC_RESOURCE_DATA_TYPE_ACCESS_MODE(HCC_DATA_TYPE_AUX(resolved_data_type)) == HCC_RESOURCE_ACCESS_MODE_WRITE_ONLY) {
			resolved_data_type |= HCC_DATA_TYPE_MUTONLY_QUALIFIER_MASK;
		}
	} else {
...
HccASTExpr* hcc_astgen_generate_ternary_expr(HccWorker* w, HccASTExpr* cond_expr) {
	...
	HccASTExpr* true_expr = hcc_astgen_generate_expr_no_comma_operator(w, 0);
	hcc_astgen_data_type_ensure_readable(w, true_expr);
	...
	HccASTExpr* false_expr = hcc_astgen_generate_expr_no_comma_operator(w, 0);
	hcc_astgen_data_type_ensure_readable(w, false_expr);
	...
	while (1) {
		...
		switch (operator_token) {
			case HCC_ATA_TOKEN_INCREMENT:
				hcc_astgen_data_type_ensure_readable(w, left_expr);
...
FIELD_ACCESS: {}
			...
			hcc_astgen_data_type_ensure_readable(w, right_expr);
...
HccDecl hcc_astgen_generate_variable_decl(HccWorker* w, bool is_global, HccDataType element_data_type, HccDataType* data_type_mut, HccASTExpr** init_expr_out) {
	...
	switch (token) {
		...
		case HCC_ATA_TOKEN_EQUAL: {
			...
			hcc_astgen_data_type_ensure_readable(w, init_expr);
			HccLocation* other_location = NULL;
	...
	switch (token) {
		...
		case HCC_ATA_TOKEN_KEYWORD_RETURN: {
			...
			if (token == HCC_ATA_TOKEN_SEMICOLON) {
				...
				hcc_astgen_data_type_ensure_readable(w, expr);
		...
		case HCC_ATA_TOKEN_KEYWORD_FOR: {
			...
			if (token != HCC_ATA_TOKEN_SEMICOLON) {
				...
				hcc_astgen_data_type_ensure_readable(w, init_stmt);
			...
			if (token != HCC_ATA_TOKEN_PARENTHESIS_CLOSE) {
				...
				hcc_astgen_data_type_ensure_readable(w, inc_stmt);
				token = hcc_ata_iter_peek(w->astgen.token_iter);
			}
