...
bool hcc_spirv_data_type_has_explicit_layout(HccDataType data_type) {
	return HCC_DATA_TYPE_IS_COMPOUND(data_type) || HCC_DATA_TYPE_IS_ARRAY(data_type);
}

bool hcc_spirv_address_space_has_explicit_layout(HccDataType data_type, HccAddressSpace address_space) {
	if (!hcc_spirv_data_type_has_explicit_layout(data_type)) {
		return false;
	}

	return address_space == HCC_ADDRESS_SPACE_BC || address_space == HCC_ADDRESS_SPACE_BUFFER;
}

HccSPIRVStorageClass hcc_spirv_storage_class(HccAddressSpace address_space) {
	switch (address_space) {
		case HCC_ADDRESS_SPACE_AUTO: return HCC_SPIRV_STORAGE_CLASS_INVALID;
		case HCC_ADDRESS_SPACE_FUNCTION: return HCC_SPIRV_STORAGE_CLASS_FUNCTION;
		case HCC_ADDRESS_SPACE_DG: return HCC_SPIRV_STORAGE_CLASS_WORK_GROUP;
		case HCC_ADDRESS_SPACE_BUFFER: return HCC_SPIRV_STORAGE_CLASS_STORAGE_BUFFER;
		case HCC_ADDRESS_SPACE_TEXTURE: return HCC_SPIRV_STORAGE_CLASS_IMAGE;
		case HCC_ADDRESS_SPACE_THREAD: return HCC_SPIRV_STORAGE_CLASS_PRIVATE;
		case HCC_ADDRESS_SPACE_BC: return HCC_SPIRV_STORAGE_CLASS_PUSH_CONSTANT;
		case HCC_ADDRESS_SPACE_UNIFORM_CONSTANT: return HCC_SPIRV_STORAGE_CLASS_UNIFORM_CONSTANT;
		case HCC_ADDRESS_SPACE_INPUT: return HCC_SPIRV_STORAGE_CLASS_INPUT;
		case HCC_ADDRESS_SPACE_OUTPUT: return HCC_SPIRV_STORAGE_CLASS_OUTPUT;
		case HCC_ADDRESS_SPACE_COUNT: return HCC_SPIRV_STORAGE_CLASS_INVALID;
	}
...
HccSPIRVId hcc_spirv_type_deduplicate(HccCU* cu, bool has_explicit_layout, HccDataType data_type) {
	...
	has_explicit_layout &= hcc_spirv_data_type_has_explicit_layout(data_type);

	HccSPIRVTypeKey key;
	HCC_ZERO_ELMT(&key);
	key.has_explicit_layout = has_explicit_layout;
	...
	if (insert.is_new) {
		...
		switch (HCC_DATA_TYPE_TYPE(data_type)) {
			case HCC_DATA_TYPE_STRUCT: {
				...
				for (uint32_t field_idx = 0; field_idx < dt->storage_fields_count; field_idx += 1) {
					...
					operands[1 + field_idx] = hcc_spirv_type_deduplicate(cu, has_explicit_layout, field->data_type);
				}
				break;
			};
			case HCC_DATA_TYPE_UNION: {
				...
				operands[1] = hcc_spirv_type_deduplicate(cu, has_explicit_layout, dt->storage_fields[dt->largest_sized_field_idx].data_type);
				operands_count = 2;
				break;
			};
			case HCC_DATA_TYPE_ARRAY: {
				...
				operands[1] = hcc_spirv_type_deduplicate(cu, has_explicit_layout, dt->element_data_type);
			...
			case HCC_DATA_TYPE_POINTER: {
				...
				HccDataType element_data_type = hcc_data_type_lower_ast_to_aml(cu, dt->element_data_type);
				bool child_has_explicit_layout = hcc_spirv_address_space_has_explicit_layout(element_data_type, dt->address_space);
				op = HCC_SPIRV_OP_TYPE_POINTER;
				operands = hcc_stack_push_many(cu->spirv.type_elmt_ids, 3);
				operands[1] = hcc_spirv_storage_class(dt->address_space);
				operands[2] = hcc_spirv_type_deduplicate(cu, child_has_explicit_layout, element_data_type);
				operands_count = 3;
			...
			case HCC_DATA_TYPE_FUNCTION: {
				...
				operands[1] = hcc_spirv_type_deduplicate(cu, false, dt->return_data_type);
				for (uint32_t param_idx = 0; param_idx < dt->params_count; param_idx += 1) {
					operands[2 + param_idx] = hcc_spirv_type_deduplicate(cu, false, dt->params[param_idx]);
				}
			...
			case HCC_DATA_TYPE_AML_INTRINSIC: {
				...
				if (HCC_AML_INTRINSIC_DATA_TYPE_COLUMNS(aml_intrinsic_type) > 1 && HCC_AML_INTRINSIC_DATA_TYPE_ROWS(aml_intrinsic_type) > 1) {
					...
					operands[1] = hcc_spirv_type_deduplicate(cu, false, HCC_DATA_TYPE(AML_INTRINSIC, vector_intrinsic_type));
					...
					operands = hcc_stack_push_many(cu->spirv.type_elmt_ids, 3);
					operands[1] = hcc_spirv_type_deduplicate(cu, false, HCC_DATA_TYPE(AML_INTRINSIC, HCC_AML_INTRINSIC_DATA_TYPE_SCALAR(aml_intrinsic_type)));
					...
					switch (aml_intrinsic_type) {
						...
						case HCC_AML_INTRINSIC_DATA_TYPE_BOOL: {
							HccSPIRVId spirv_id = hcc_spirv_type_deduplicate(cu, false, HCC_DATA_TYPE_AML_INTRINSIC_S32);
			...
			case HCC_DATA_TYPE_RESOURCE: {
				HccSPIRVId spirv_id = hcc_spirv_type_deduplicate(cu, false, HCC_DATA_TYPE_AML_INTRINSIC_U32);
				atomic_store(&entry->spirv_id, spirv_id);
				return spirv_id;
			};
			...
			case HCC_DATA_TYPE_RESOURCE_DESCRIPTOR: {
				...
				switch (HCC_RESOURCE_DATA_TYPE_TYPE(resource_data_type)) {
					case HCC_RESOURCE_DATA_TYPE_BUFFER: {
						...
						HccDataType element_data_type = hcc_data_type_lower_ast_to_aml(cu, d->element_data_type);
						bool child_has_explicit_layout = hcc_spirv_address_space_has_explicit_layout(element_data_type, HCC_ADDRESS_SPACE_BUFFER);
						HccSPIRVId element_spirv_id = hcc_spirv_type_deduplicate(cu, child_has_explicit_layout, element_data_type);
					...
					case HCC_RESOURCE_DATA_TYPE_TEXTURE:
						...
						operands_count = 8;
						operands[1] = hcc_spirv_type_deduplicate(cu, false, HCC_DATA_TYPE(AML_INTRINSIC, sample_data_type));
		...
		if (needs_deduplication) {
			...
			HCC_ZERO_ELMT(&unique_key);
			unique_key.operands = operands + 1;
			unique_key.operands_count = operands_count - 1;
			unique_key.has_explicit_layout = has_explicit_layout;
			unique_key.op = op;
		...
		if (!needs_deduplication || unique_insert.is_new) {
			...
			if (!HCC_DATA_TYPE_IS_AML_INTRINSIC(data_type) && !HCC_DATA_TYPE_IS_POINTER(data_type)) {
			...
			if (HCC_DATA_TYPE_IS_COMPOUND(data_type)) {
				...
				HccPointerDataType* dt = hcc_pointer_data_type_get(cu, data_type);
				if (dt->address_space == HCC_ADDRESS_SPACE_BC || dt->address_space == HCC_ADDRESS_SPACE_INPUT || dt->address_space == HCC_ADDRESS_SPACE_OUTPUT) {
					HccDataType element_data_type = hcc_data_type_lower_ast_to_aml(cu, dt->element_data_type);
					if (HCC_DATA_TYPE_IS_COMPOUND(element_data_type)) {
						HccSPIRVOperand elmt_data_type_spirv_id = operands[2];
						operands = hcc_spirv_add_decorate(cu, 2);
						operands[0] = elmt_data_type_spirv_id;
	...
	if (insert.is_new) {
		...
		switch (HCC_DECL_TYPE(decl)) {
			case HCC_DECL_GLOBAL_VARIABLE: {
				...
				operands[0] = hcc_spirv_type_deduplicate(cu, false, hcc_pointer_data_type_deduplicate(cu, ast_global_variable->data_type, HCC_ADDRESS_SPACE_THREAD));
				operands[1] = spirv_id;
...
void hcc_spirv_resource_descriptor_binding_deduplicate(HccCU* cu, HccDataType data_type, HccSPIRVDescriptorBindingInfo* info_out) {
	...
	HccResourceDataType resource_data_type = HCC_DATA_TYPE_AUX(data_type);
	HccAddressSpace address_space = HCC_RESOURCE_DATA_TYPE_TYPE(resource_data_type) == HCC_RESOURCE_DATA_TYPE_BUFFER ? HCC_ADDRESS_SPACE_BUFFER : HCC_ADDRESS_SPACE_UNIFORM_CONSTANT;
	HccSPIRVId data_type_spirv_id = hcc_spirv_type_deduplicate(cu, false, data_type);
	HccSPIRVId data_type_ptr_spirv_id = hcc_spirv_type_deduplicate(cu, false, hcc_pointer_data_type_deduplicate(cu, data_type, address_space));
	...
	if (insert.is_new) {
		...
		HccSPIRVStorageClass storage_class = hcc_spirv_storage_class(address_space);
...
HccSPIRVId hcc_spirv_constant_deduplicate(HccCU* cu, HccConstantId constant_id) {
	...
	if (insert.is_new) {
		...
		operands[0] = hcc_spirv_type_deduplicate(cu, false, final_data_type);
		operands[1] = spirv_id;
...
bool hcc_spirv_type_key_cmp(void* a, void* b, uintptr_t size) {
	...
	return a_->has_explicit_layout == b_->has_explicit_layout && a_->data_type == b_->data_type;
}
...
bool hcc_spirv_unique_type_key_cmp(void* a, void* b, uintptr_t size) {
	...
	return a_->op == b_->op && a_->operands_count == b_->operands_count && a_->has_explicit_layout == b_->has_explicit_layout && memcmp(a_->operands, b_->operands, a_->operands_count * sizeof(HccSPIRVOperand)) == 0;
}
...
HccHash hcc_spirv_unique_type_key_hash(void* key, uintptr_t size) {
	...
	hash = hcc_hash_fnv(&k->has_explicit_layout, sizeof(k->has_explicit_layout), hash);
