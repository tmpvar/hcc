...
build\hcc.exe -O -g -fi samples/shaders.c -fo samples/shaders.spirv -fomc samples/shaders-metadata.h --enable-unordered-swizzling && ^
...
IF "%~1" == "release" (
  ...
  cd build
  powershell Compress-Archive -Force -DestinationPath hcc-0.4.0-windows.zip -LiteralPath hcc.exe, hcc.pdb, ..\libc, ..\libhmaths, ..\libhccintrinsics, ..\interop, ..\samples, ..\playground, ..\docs, ..\README.md, ..\LICENSE
  cd ..
)
