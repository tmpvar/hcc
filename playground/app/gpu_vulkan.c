...
bool gpu_reload_shaders(void) {
...

	{
		void* code;
		uintptr_t code_size;
		if (!platform_file_read_all(APP_SHADERS_PATH, &code, &code_size)) {
			return false;
		}
		...
		APP_VK_ASSERT(vkCreateShaderModule(gpu.device, &create_info, NULL, &shader_module));
	}
	...
	gpu.shader_module = shader_module;
